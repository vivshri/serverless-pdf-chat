...
#     file_name_full = key.split("/")[-1]
    ...
    index_from_loader.vectorstore.save_local("/tmp/fsiss")

    s3.upload_file(
        "/tmp/fsiss/index.faiss", BUCKET, f"{user_id}/{file_name_full}/index.faiss"
    )
    s3.upload_file("/tmp/fsiss/index.pkl", BUCKET, f"{user_id}/{file_name_full}/index.pkl")
